...
<div class="row">
	<div class="col-lg-6">
		...
		<div class="box box-info">
			<form method="POST" action="/admin/control-definitions/{{ $control_definition->id }}" class="needs-validation" novalidate>
				{{ csrf_field() }}
				{{ method_field('PATCH') }}
				<div class="box-body">
					<div class="form-group">
						...
						<select 
							...
							@foreach ($users as $user)
								<option value="{{ $user->id }}" {{ $user->id == $control_definition->user_id ? 'selected' : null }}>{{ $user->name }}</option>
							@endforeach
						</select>
					</div>
