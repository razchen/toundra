...
<div class="row">
	<div class="col-lg-8">
		...
		<form method="POST" action="/admin/scenes/{{ $scene->id }}" class="needs-validation" novalidate>
			{{ csrf_field() }}
			{{ method_field('PATCH') }}
					...
					<div class="form-group">
						...
						<select class="form-control" name="user_id">
							@foreach($users as $user)
								<option value="{{ $user->id }}" {{ $scene->user_id == $user->id ? 'selected' : null }}>{{ $user->id }} - {{ $user->name }}</option>
							@endforeach
						</select>
					</div>
